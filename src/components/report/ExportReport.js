...
const ExportReport = () => {
    ...
    const fetchData = () => {
        ...
        let url = (timeType === 'single') ? `${process.env.REACT_APP_API_URL}/export-report?month=${month}&year=${year}` :
            `${process.env.REACT_APP_API_URL}/export-report?fromMonth=${fromMonth}&fromYear=${fromYear}&toMonth=${toMonth}&toYear=${toYear}`
        axios.get(url)
            .then((response) => {
